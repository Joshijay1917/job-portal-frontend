...
export function LatestJobs() {
    ...
    const handleApply = async (jobId: string) => {
        ...
        await usecandidate.applyJobPost(jobId)
                        ...
                        <div key={job._id} className='bg-white rounded-xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow duration-200 p-5 md:p-6'>
                            ...
                            <div className='w-full flex-col md:flex-row gap-3 flex md:items-center md:justify-between'>
                                ...
                                <button
                                    ...
                                    className='bg-blue-600 w-full md:w-auto text-white px-5 py-2.5 rounded-lg hover:bg-blue-700 cursor-pointer transition-colors font-medium text-sm inline-flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed'
                                >
                                    {applyingJobId === job._id && <span className="h-4 w-4 border-2 border-white border-t-transparent rounded-full animate-spin" />}
                                    Apply Job
                                </button>
                            </div>
                        </div>
